...
; :Author: han
;-
pro cross_match_lut_qiucatalog_survey
close,/all
path_cat='E:\Data\lut_data\survey_cat\cat\'
lut_file = path_cat + 'lutcat_20140312090404.cat'
path='E:\Data\lut_data\survey_cat\'
qiucata_file = path + 'nlp_60.cat'
...
xtitle='Observational magnitude (20 FWHM)',$
XCHARSIZE=1.2,$
...
if(md[index[k]] le (3.0/3600.)) and ( lut_m20[index[k]] le 15.5 ) then begin
...
;print,match_num
for k=0,maxrec1-1 do begin  
  oplot,[match_obj[10,k],match_obj[10,k]],$
    [match_obj[5,k],match_obj[5,k]],$
     psym=4,thick=2,$
     color=2
...
;  if(abs(match_obj[6,match_ord[k]] - match_obj[14,match_ord[k]])) ge 1.0 then begin
  if(match_obj[5,k] - match_obj[10,k]) ge 1.0 then begin
  oplot,[match_obj[10,k],match_obj[10,k]],$
    [match_obj[5,k],match_obj[5,k]],$
     psym=4,thick=3,$
     color=1
