...
; :Author: han
;-
pro cross_match_lut_qiucatalog_survey_batch
close,/all
...
match_file = path + 'match_qiu.txt'
no_match_file = path + 'no_match_qiu.txt'
...
' XCENTER         YCENTER         MAG2FULL       MAG2FULLERR   ' + $
'JD        DATETIME               DIRIMG '
flare_file = path + 'lvs_qiu.txt'
openw,90,flare_file,width = 3000
printf,90,'NAME                    RA              DEC             SNR2           '+$
' XCENTER         YCENTER         MAG2FULL       MAG2FULLERR   ' + $
'JD        DATETIME               DIRIMG                  nlp_ra          nlp_dec          '+$
'nlp_ab           nlp_id      angula_distance(s) '
output_file = path + 'LUT_survey_mag_qiu'
...
;  if(abs(match_obj[5,k] - match_obj[10,k])) ge 1.0 then begin
  if(match_obj[10,k] - match_obj[5,k]) ge 1.2 and match_obj[5,k] le 11 then begin
  oplot,[match_obj[5,k],match_obj[5,k]],$
